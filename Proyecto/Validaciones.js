...
function setupHorarioRecurrente() {
    ...
    const franjasContainer = container.querySelector('#franjasPorDia');

    function renderDayArea(dia) {
        let area = franjasContainer.querySelector(`[data-dia="${dia}"]`);
        if (!area) {
            area = document.createElement('div');
            area.dataset.dia = dia;
            area.className = 'dia-franjas';
            area.innerHTML = `
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
                    <strong style="width:60px;text-transform:capitalize">${dia}</strong>
                    <button type="button" class="add-franja" data-dia="${dia}" style="padding:6px 8px">+ Añadir franja</button>
                </div>
                <div class="franjas-list" style="display:flex;flex-direction:column;gap:6px"></div>
            `;
            franjasContainer.appendChild(area);
            area.querySelector('.add-franja').addEventListener('click', () => addFranjaUI(dia));
        }
        return area;
    }
    ...
    diasInputs.forEach(inp => {
        inp.addEventListener('change', function() {
            ...
            if (this.checked) {
                ...
                const a = franjasContainer.querySelector(`[data-dia="${dia}"]`);
                if (a) a.remove();
            }
        });
    });
...
function validarCitas() {
    ...
    if (fechaCita < ahora) {
        mostrarMensaje('La fecha de la cita no puede ser pasada', 'error');
        return false;
    }
...
function renderListaMedicos() {
    renderizarListaDatos({
        ...
        formatoCampos: m => {
            ...
            if (m.horarioDesde || m.horarioHasta) {
                const desde = m.horarioDesde ? new Date(m.horarioDesde).toLocaleString() : '-';
                const hasta = m.horarioHasta ? new Date(m.horarioHasta).toLocaleString() : '-';
                campos.push(['Horario', `${desde} → ${hasta}`]);
            }
            return campos;
        }
    });
...
function editarPaciente(id) { editRegistro('pacientes', id); }
function eliminarPaciente(id) { deleteRegistro('pacientes', id); }
function eliminarRegistro(tipo, id) { deleteRegistro(tipo, id); }
...
function actualizarTodoTrasCarga() {
    ...
    if (formCitas && !formCitas.__validaciones_attached) {
        formCitas.addEventListener('submit', function(e) {
            ...
            try {
                ...
                console.error('Error manejando submit de formCitas:', err);
            }
        });
        formCitas.__validaciones_attached = true;
    }
